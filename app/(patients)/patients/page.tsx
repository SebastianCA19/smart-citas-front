  ...
  const fetchAppointments = async (cedula: number) => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        sessionStorage.setItem('appointments', JSON.stringify(parsedAppointments));

        const month = new Date().getMonth() + 1;
        const year = new Date().getFullYear();

        const filteredAppointments = parsedAppointments.filter((apt: Appointment) => {
      ...
      <div className="min-h-screen bg-slate-50 p-6 lg:p-10">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-blue-900 border-b-2 border-b-neutral-200 pb-4">
            CITAS PENDIENTES DE {new Date().toLocaleString('default', { month: 'long' }).toUpperCase()}
          </h1>
          <a
